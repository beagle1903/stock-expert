---
name: run
description: Start or reuse and observe the Stock Expert web app, verify its UI, API, proxy, logs, and persisted dashboard semantics for at least five minutes, then open it in Cursor's in-app browser. Use when the user says /run, stock-expert:run, run the web app, open the dashboard, or asks to launch the Stock Expert frontend.
---

# Stock Expert Web App

Start the local Stock Expert web app and open it in Cursor's in-app browser.
...
Use Cursor's in-app browser (`cursor-ide-browser`) to open
`http://127.0.0.1:5173/`. Do not open an external browser. Inspect
browser-console errors when the browser exposes them; this is complementary to
the launcher-owned watchdog. Keep healthy servers running, then report the URL,
direct/proxied API health, started/reused state, component process ids,
observation duration/poll count, and final watchdog result.
