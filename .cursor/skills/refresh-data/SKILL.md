...
4. Use Cursor's in-app browser for every Investing.com interaction. Do not
...
the rendered table by page state, and capture all four tabs. Validate the
capture and publish the bundle atomically. The repository CLI launcher is not
an in-app browser adapter and must not be used for this workflow. Do not bypass
CAPTCHA, Cloudflare, or another access challenge. When a challenge appears, ask
the user to complete it in the in-app browser.
