...
- Snapshot metadata, market rows, price rows, and captured provenance (coverage, skip counts, mapping-failure names) commit atomically; failed imports leave the previous latest snapshot active. `mapped_count` is the count of rows that passed required numeric validation and were persisted. Older non-CSV `create_snapshot_run` rows stay `not_captured`. The Evidence Console Snapshots view is read-only lineage, not a daily command.
