...
- `/api/strategy-playback/{review_id}` assembles one point-in-time review from immutable pick results, review-owned candidate outcomes, its exact signal snapshot, the latest persisted weights effective on the signal date, and exact-snapshot pilot sessions. Post-review weights are not mislabeled as basket-generation inputs; a pilot comparison is available only when both expected arms are present and complete, while missing optional evidence remains partial or unavailable.
