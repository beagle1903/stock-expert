...
- Retries cover HTTP 429 (`Retry-After` or backoff), transient network errors, and malformed JSON. Empty/error chart payloads fail that ticker without retry.
- CSV export stays under `data/` and cannot overwrite `fiyat.csv`, `performans.csv`, `teknik.csv`, or `temel.csv`.
- A download that produces zero rows, or any ticker failure while a Yahoo CSV already exists, leaves that file in place and does not write SQLite.
